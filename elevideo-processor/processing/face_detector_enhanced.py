...
class EnhancedFaceTracker:
    ...
    def get_quality_metrics(self) -> TrackingQuality:
        ...
        stability = (
            1.0 / (1.0 + np.std([h[0] for h in list(self.history)[-10:]]) / 10.0)
        ...
        return TrackingQuality(
            ...
            is_reliable=(
                avg_confidence > 0.7
                and stability > 0.6
                and self.lost_frames < 5
                and self.consecutive_detections > 2
            ),
        )
...
class EnhancedFaceDetector:
    ...
    def __init__(self, config):
        ...
        self.total_detection_failures = 0
        ...
        self.use_fallback_detection = config.FACE_DETECTION.get("use_fallback_detection", True)
        self.max_lost_frames = config.FACE_DETECTION.get("tracking_max_lost_frames", 15)
    ...
    def detect(self, frame) -> List[dict]:
        ...
        mediapipe_detected = bool(raw_faces)
        ...
        if mediapipe_detected:
            ...
            # Si ya existe un tracker, intentar recuperación visual inmediatamente.
            # Esperar decenas de muestras permite que una pérdida breve termine cortando
            # la cara antes de que el fallback tenga oportunidad de ayudar.
            if self.trackers and self.use_fallback_detection:
                fallback_faces = self._detect_with_fallback(frame)
                if fallback_faces:
                    raw_faces = fallback_faces
        ...
        self.trackers = [
            t for t in self.trackers if t.is_alive(self.max_lost_frames)
        ]
    ...
    def get_primary_face(self, faces) -> Optional[dict]:
        ...
        reliable = [
            f for f in faces
            if f.get("quality") and f["quality"].is_reliable
        ]
    ...
    def get_tracking_stats(self) -> dict:
        return {
            ...
            "total_detection_failures": self.total_detection_failures,
            "fallback_activations": self.fallback_activations,
            "fallback_successes": self.fallback_successes,
            "reliable_trackers": sum(
                1 for t in self.trackers if t.get_quality_metrics().is_reliable
            ),
        }
    ...
    def _detect_mediapipe(self, frame) -> List[dict]:
        ...
        result = self.mp_face.process(
            cv2.cvtColor(detect_frame, cv2.COLOR_BGR2RGB)
        )
        faces = []
    ...
    def _detect_with_fallback(self, frame) -> List[dict]:
        ...
        for tracker in self.trackers:
            ...
            # El radio escala con el tamaño del rostro para tolerar movimiento rápido,
            # pero mantiene un techo razonable para evitar enganchar otra región de piel.
            association_radius = float(np.clip(max(pw, ph) * 1.25, 120, 240))
            ...
            for sx, sy, sw, sh in skin_regions:
                ...
                if d < min_dist and d < association_radius:
                    min_dist, best = d, (sx, sy, sw, sh)

            if best:
                x, y, bw, bh = best
                confidence = max(0.5, 1.0 - min_dist / association_radius)
                faces.append({
                    "bbox": (x, y, bw, bh),
    ...
    def _is_valid(self, x, y, w, h, fw, fh) -> bool:
        ...
        if not (0.4 <= ar <= 2.5):
            return False
        if x < -w * 0.3 or y < -h * 0.3 or x + w > fw + 5 or y + h > fh + 5:
            return False
        return True
    ...
    def _update_trackers(self, detections, fw, fh):
        ...
        for i, det in enumerate(detections):
            ...
            for j, tracker in sorted_t:
                ...
                score = (
                    iou * 0.7
                    + (1.0 - dist / np.hypot(fw, fh)) * 0.3
                )
                if score > best_score:
                    best_score, best_j = score, j

            if best_j is not None:
                self.trackers[best_j].update(
                    det["bbox"],
                    det["confidence"],
                )
    ...
    def _apply_temporal_smoothing(self, faces) -> List[dict]:
        ...
        for i, face in enumerate(faces):
            q = face.get("quality")
            window = 3 if (q and q.is_reliable) else min(
                7,
